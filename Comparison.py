# -*- coding: utf-8 -*-
"""
Perform feature comparison of houshold survey data
...
# =============================================================================
# Source Custom Comparison Algorithms
# =============================================================================
exec(open(path_code + 'Custom Comparison Algorithms.py').read())
# =============================================================================
# Import DATA
# =============================================================================
...
featCompFile = path + run_date + '/featComp' + run_date + '.txt'
featComp.to_csv(featCompFile, index=False, sep = '\t')

with open(path + run_date + '/compareDict' + run_date + '.txt', 'w') as f:
    f.write(json.dumps(timeDict))
    
with open(path + run_date + '/errorDict' + run_date + '.txt', 'w') as f:
    f.write(json.dumps(errorMSGDict))
    
codebook_lines = ['{:20} {:105}\n'.format(key + ':', value) for key, value in codebook.items()]
file = open(path + run_date + '/featComp_codebook' + run_date + '.txt', 'w')
